...
// import { invoke } from '@tauri-apps/api/core'; // Removed for web version
...
export function ModelSettingsModal({
  ...
  const fetchApiKey = async (provider: string) => {
    // Web version: skip API key fetching from Tauri keystore
    // We rely on what's passed in modelConfig or let user enter it manually
    setApiKey('');
  };
  ...
  useEffect(() => {
    const fetchModelConfig = async () => {
      ...
      try {
        // Mock data or rely on initial props for web version
        // const data = (await invoke('api_get_model_config')) as any;
        // setModelConfig...
        // For web, we assume config is passed via props or available via other means
        // Just marking as loaded
        hasLoadedInitialConfig.current = true;
      } catch (error) {
  ...
  useEffect(() => {
    const fetchAutoGenerateSetting = async () => {
      // Web version: assume auto-generate is enabled or handled by backend default logic
  ...
  const fetchOllamaModels = async (silent = false) => {
    ...
    try {
      ...
      // Web version: Mock logic - we assume some default models or empty list if we can't connect to Ollama directly
      // In a real web app, the backend should proxy this request. 
      // For now, we simulate finding the gemma model if simple string matches? 
      // Actually we can't easily "fetch" from client side to Ollama on user machine w/o backend proxy if CORS issues.
      // Let's assume emptiness or a placeholder.
      toast.info("Note: Listing Ollama models requires backend connection in Web Version. Assuming models exist.");

      const mockModels: OllamaModel[] = [{
        name: 'gemma3:1b', id: 'gemma3:1b', size: 'unknown', modified: new Date().toISOString()
      },
      {
        name: 'gemma2:2b', id: 'gemma2:2b', size: 'unknown', modified: new Date().toISOString()
      }];

      const modelList = mockModels; // (await invoke('get_ollama_models', { endpoint })) as OllamaModel[];
  ...
  const downloadRecommendedModel = async () => {
    ...
    try {
      toast.error("Downloading models directly is not supported in Web Version. Please install models manually on the server.");
    } catch (err) {
  ...
  const deleteOllamaModel = async (modelName: string) => {
    try {
      toast.error("Deleting models is not supported in Web Version.");
    } catch (err) {

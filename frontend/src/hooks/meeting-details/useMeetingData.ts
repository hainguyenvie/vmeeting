...
export function useMeetingData({ meeting, summaryData, onMeetingUpdated }: UseMeetingDataProps) {
  ...
  // 🔥 FIX: Sync transcripts when meeting.transcripts changes (e.g., after refresh)
  useEffect(() => {
    if (meeting.transcripts && Array.isArray(meeting.transcripts)) {
      console.log('📊 useMeetingData: Syncing transcripts from props:', meeting.transcripts.length, 'items');
      setTranscripts(meeting.transcripts);
    }
  }, [meeting.transcripts]);

  // Sync aiSummary when summaryData changes
  useEffect(() => {
  ...
  const handleSaveSummary = useCallback(async (summary: Summary | { markdown?: string; summary_json?: any[] }) => {
    ...
    try {
      ...
      await meetingsAPI.update(meeting.id, {
        summary: JSON.stringify(formattedSummary),
      });
  ...
  return {
    ...
    // Setters
    setTranscripts, // ✅ ADD: Expose setTranscripts for speaker management
    setMeetingTitle,
    setIsEditingTitle,
    setAiSummary,
    setIsSummaryDirty,
